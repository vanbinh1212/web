...
class account {
	...
	public static function create($email, $password, $phone, $fullname) {
		...
		if(sqlsrv_num_rows($qCheck) <= 0) {
			...
			$qInsert = sqlsrv_query($conn, "insert into Mem_Account (Email, Password, Fullname, Phone, Money, TimeCreate, IPCreate, MoneyLock) VALUES (?, ?, ?, ?, 0, ?, ?, ?)", array($email, $password, $fullname, $phone, time(), self::getUserIP(), 0));
	...
	public static function getMoney($username) {
		...
		global $conn, $_config;
		if($username == null) return $username;
		$qCheck = sqlsrv_query($conn, "select Money from Mem_Account where Email = ?", array($username), array( "Scrollable" => SQLSRV_CURSOR_KEYSET));
	...
	public static function getPhone($username) {
		...
		global $conn, $_config;
		if($username == null) return $username;
		$qCheck = sqlsrv_query($conn, "select Phone from Mem_Account where Email = ?", array($username), array( "Scrollable" => SQLSRV_CURSOR_KEYSET));
	...
	public static function getPassTwoPlayer($username) {

		global $conn_sv, $_config;
		
		if($username == null) return $username;
		
		$qCheck = sqlsrv_query($conn_sv, "SELECT PasswordTwo FROM Sys_Users_Detail WHERE UserName = ?", array($username), array( "Scrollable" => SQLSRV_CURSOR_KEYSET));
	...
	public static function changePasswordforLauncher($userid, $newpassword) {
	...
	public static function findMomoByTransId($transid) {
		global $conn, $_config;
		...
		if(sqlsrv_num_rows($qFind) > 0) {
			...
			$momoInfo = sqlsrv_fetch_array($qFind, SQLSRV_FETCH_ASSOC);
	...
	public static function getListPlayerInfoForLauncher($username) {

		global $conn_sv, $_config;
		
		$listAccount = array();

		$qCheck = sqlsrv_query($conn_sv, "select UserID, NickName, Grade from Sys_Users_Detail where UserName = ? AND IsExist = ?", array($username, true), array( "Scrollable" => SQLSRV_CURSOR_KEYSET));
